...
- Post-feedback correction on May 7: the titlebar still looked too short because a hard horizontal separator was treated as the perceived titlebar bottom. RelayDock now hides the AppKit transparent-titlebar separator and removes the SwiftUI `ShellTopBar` bottom `Divider`; visual QA also checks that no hard separator crosses the content-pane top bar.
